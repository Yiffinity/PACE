...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        # the same runtime IPC socket and one fails
    ...
    async def generate(
        ...
        try:
            ...
            # Some vLLM versions expose StructuredOutputsParams but do not yet
            # accept it in SamplingParams. Retry with the legacy field.
            if json_schema is None or "structured_outputs" not in sampling_params:
                raise
            sampling_params.pop("structured_outputs", None)
            sampling_params["guided_json"] = json_schema
            try:
                sampling_params = SamplingParams(max_tokens=max_tokens, **sampling_params)
