...
from verl.workers.rollout.vllm_rollout.utils import _ipc_socket_uri, get_device_uuid
...
class ServerAdapter(BaseRollout):
    ...
    def __init__(
        ...
        # stale socket file) cannot collide in the shared temporary directory.
        local_rank = self.rollout_rank % local_world_size
        job_id = ray.get_runtime_context().get_job_id()
        self.zmq_handle = _ipc_socket_uri(
            f"rl-colocate-zmq-{job_id}-replica-{self.replica_rank}-rank-{local_rank}.sock"
        )
