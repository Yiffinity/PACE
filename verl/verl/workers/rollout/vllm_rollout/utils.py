...
def _ipc_socket_uri(name: str) -> str:
    root = Path(os.environ.get("VERL_IPC_ROOT", tempfile.gettempdir())).expanduser().resolve()
    root.mkdir(parents=True, exist_ok=True)
    return f"ipc://{root / name}"
...
class vLLMColocateWorkerExtension:
    ...
    def _get_zmq_handle(self) -> str:
        ...
        return _ipc_socket_uri(
            f"rl-colocate-zmq-{job_id}-replica-{replica_rank}-rank-{self.local_rank}.sock"
        )
