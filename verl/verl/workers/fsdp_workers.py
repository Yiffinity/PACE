...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if image_text_auto is not None and type(actor_model_config) in image_text_auto._model_mapping.keys():
                actor_module_class = image_text_auto
            ...
            elif getattr(actor_model_config, "vision_config", None) is not None:
                raise RuntimeError(
                    f"Transformers cannot load multimodal model type {actor_model_config.model_type!r}; "
                    "install a release that supports its image-to-text architecture"
                )
            else:
            ...
            if role == "ref":
                ...
                actor_module.eval()
                if any(parameter.requires_grad for parameter in actor_module.parameters()):
                    raise RuntimeError("reference teacher parameters must remain frozen")
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ref_model_path = self.config.model.get("ref_model_path")
