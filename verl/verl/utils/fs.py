...
def copy_to_shm(src: str):
    """
    Load the model into the configured shared-memory cache directory.
    """
    shm_model_root = os.environ.get("VERL_SHM_CACHE_DIR")
    if not shm_model_root:
        raise ValueError("Set VERL_SHM_CACHE_DIR to a shared-memory directory when use_shm is enabled")
    shm_model_root = os.path.abspath(os.path.expanduser(shm_model_root))
